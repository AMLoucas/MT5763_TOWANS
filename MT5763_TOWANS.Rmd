...
When applying the simulation, we want our sample data to have the same number rows as the original dataset. In the provided code the number of rows are obtained using the code below: 
```{eval=FALSE}
...
is indeed more efficient. The times have a huge difference, for the large values of N we can see a very big difference. The supplied bootstrap took 23 minutes to execute N = 50000, while the updated version needed only 3.69 seconds. 
...
RUN;
```
Below you can find the 95% confidence intervals on the origianl (non-bootstrap) data, on the regBoot data and on the reBootUpdated data. 
...
Our estimate values from the original data without bootstrap sampling are for Intercept *96.153* and for Slope *2.036* . We can see that the confidence intervals from Bootstrap algorithms contain the values and is approximately located in the middle of both of the histograms. This indicates our bootstrap results are "good". Additionally we notice that 95% confidence intervals for all 3 methods differ in values and width. The reason for this phenomeno is because the estimates and confidence intervals are dependent on there data. For our original data without bootstrap, the values will always be the same, but for the bootstrap methods the values will always change. This is due to the fact that every time we run the re-sampling software, new random sample data is constructed, resulting to different results of our estimates and confidence intervals. In general, both codes do the same procedure, the only difference is that regBootUpdated does it in a more efficient way. To obtain more precise confidence intervals, you will have to increase the number of simulations *N* . The larger the value of *N* the closer our confidence intervals will be to the true parameter. 
...
From the graph above, we can see that there seems to be a linear relation between the sample variance and inverse of number of Monte Carlo simulation.
